...
#[test]
fn new() {
    let db = Database::new("data/_new").unwrap();
    assert_eq!(db.len(), 0);
}

#[test]
fn create_collection() {
    let mut db = Database::new("data/_create_collection").unwrap();
...
#[test]
fn get_collection() {
    let db = create_test_database("data/_get_collection");
    let collection: Collection<usize, 128, 32> =
...
fn save_collection_update() {
    let mut db = create_test_database("data/_save_collection_update");
...
fn delete_collection() {
    let mut db = create_test_database("data/_delete_collection");
